...
export async function POST(req: NextRequest) {
  const { amount, description, fiat_currency = "UAH", nick, email, product, productId, tier, tierDuration } = await req.json();
  ...
  const payload = JSON.stringify({ nick, email, product, productId, tier, tierDuration, uah: amount });
